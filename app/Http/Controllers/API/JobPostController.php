...
{
    /**
     * Display jobs for employer with matches & applications count
     */
    public function indexForEmployer($employerId): JsonResponse
    {
        Log::info('Entering indexForEmployer', ['employer_id' => $employerId]);

        try {
            Log::debug('Fetching jobs for employer', ['employer_id' => $employerId]);
    ...
    /**
     * Get candidates matching job title (case-insensitive)
     */
    private function getMatchingCandidates(JobPosting $job)
    {
        ...
        Log::debug('Sample matched candidates', [
            ...
            'sample_matches' => $sampleMatches->map(fn($c) => [
                'id' => $c->id,
                'name' => $c->full_name,
    ...
    /**
     * Job dashboard - matches + applications with contact masking
     */
    ...
    // Fields to log (in order of your $rules array)
    $fields = [
        'employer_id',
        'company_id',
        'company_name',
    ...
    foreach ($fields as $field) {
        if ($field === 'pan_card') {
            $logData['pan_card'] = $request->hasFile('pan_card')
                ? [
                    'uploaded'     => true,
                    'filename'     => $request->file('pan_card')->getClientOriginalName(),
                    'size_kb'      => round($request->file('pan_card')->getSize() / 1024, 2),
                    'mime_type'    => $request->file('pan_card')->getMimeType(),
                  ]
                : ['uploaded' => false];
        } else {
            $logData[$field] = $request->input($field);
        }
    }

    Log::info('JOB POSTING - STORE REQUEST RECEIVED', $logData);
        ...
        $rules = [
            ...
            'work_location_type' => 'nullable',
            'interview_pref_loca' => 'required',
        ...
        try {
            ...
            if (!$employer->hasEnoughCredits(1, 'job_post')) {
                return response()->json([
                    'status' => 'error',
            ...
            Mail::to('manshu.developer@gmail.com')->send(new AdminNewJobPosted([
                'employer' => $employer,
    ...
    /**
     * Get all jobs by employer (simple list)
     */
    ...
    public function refreshJob($jobId): JsonResponse
    {
        try {
            ...
            if (!$jobPosting) {
                return response()->json(['status' => 'error', 'message' => 'Job posting not found or unauthorized'], 404);
            }

            if (!$employer->hasEnoughCredits(1)) {
                return response()->json([
                    'status' => 'error',
    ...
    public function update(Request $request, $jobId): JsonResponse
    {
        ...
        try {
            ...
            if ($request->company_name) {
                ...
                if (!$company) {
                    return response()->json(['status' => 'error', 'message' => 'Invalid or unauthorized company.'], 422);
                }
                $company_id = $company->id;
            }
    ...
    /**
     * Apply additional filters (unused in current code but kept)
     */
    ...
    /**
     * Format salary (e.g., 500000 → 5L, 1200000 → 12L, etc.)
     */
    private function formatSalary($salary)
    {
        ...
        if ($salary >= 100000) {
            return round($salary / 100000, 1) . 'L';
        }
    ...
    /**
     * Show single job by slug (public view)
     */
    public function show($slug): JsonResponse
    {
        ...
        $showContactDetails = !in_array($commPref, ["No, I will contact candidates first"]);
        ...
        $formatted = [
            ...
            "About Company" => $showContactDetails ? [
